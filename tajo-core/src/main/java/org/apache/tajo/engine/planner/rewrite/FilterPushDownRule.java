...
public class FilterPushDownRule extends BasicLogicalPlanVisitor<FilterPushDownContext, LogicalNode>
  ...
  public LogicalPlan rewrite(LogicalPlan plan) throws PlanningException {
    ...
    for (LogicalPlan.QueryBlock block : plan.getQueryBlocks()) {
      context.clear();
      this.visit(context, plan, block, plan.getPlanTree(), block.getRoot(), new Stack<LogicalNode>());
    }
...
//    visit(context, plan, block, selNode.getChild(), stack);
    visit(context, plan, block, planTree, planTree.getChild(selNode), stack);
    stack.pop();
...
//      }
      if (ArityClass.getArityClassByNodeType(node.getType()) == ArityClass.UNARY) {
...
//          if (((RelationNode)joinNode.getRightChild()).getCanonicalName().equals(rightTableName)) {
    ...
    if (LogicalPlanner.isOuterJoin(joinNode.getJoinType())) {
      ...
      if (joinNode.getJoinType() == JoinType.RIGHT_OUTER) {
        nullSupplyingTableNameSet = TUtil.newHashSet(PlannerUtil.getRelationLineage(planTree,
            planTree.getLeftChild(joinNode)));
      } else {
        nullSupplyingTableNameSet = TUtil.newHashSet(PlannerUtil.getRelationLineage(planTree,
            planTree.getRightChild(joinNode)));
      ...
      if (joinNode.getJoinType() == JoinType.RIGHT_OUTER) {
        preservedTableNameSet = TUtil.newHashSet(PlannerUtil.getRelationLineage(planTree,
            planTree.getRightChild(joinNode)));
      } else {
        preservedTableNameSet = TUtil.newHashSet(PlannerUtil.getRelationLineage(planTree,
            planTree.getLeftChild(joinNode)));
...
//    LogicalNode right = joinNode.getRightChild();
    ...
    visit(context, plan, block, planTree, left, stack);
    ...
    visit(context, plan, block, planTree, right, stack);
  ...
  @Override
  public LogicalNode visitUnion(FilterPushDownContext context, LogicalPlan plan, LogicalPlan.QueryBlock block,
                                LogicalPlanTree planTree, UnionNode unionNode,
                                Stack<LogicalNode> stack) throws PlanningException {
//    LogicalNode leftNode = unionNode.getLeftChild();
    LogicalNode leftNode = planTree.getLeftChild(unionNode);
...
//    LogicalNode rightNode = unionNode.getRightChild();
    LogicalNode rightNode = planTree.getRightChild(unionNode);
    transformedMap = transformEvalsWidthByPassNode(origins, plan, block, unionNode, rightNode);
    context.setFiltersTobePushed(new HashSet<EvalNode>(transformedMap.keySet()));
    visit(context, plan, plan.getBlock(rightNode), planTree, rightNode, stack);
  ...
  public LogicalNode visitProjection(FilterPushDownContext context,
                                     LogicalPlan plan,
                                     LogicalPlan.QueryBlock block,
                                     LogicalPlanTree planTree,
                                     ProjectionNode projectionNode,
                                     Stack<LogicalNode> stack) throws PlanningException {
//    LogicalNode childNode = projectionNode.getChild();
    LogicalNode childNode = planTree.getChild(projectionNode);
    ...
    LogicalNode current = visit(context, plan, plan.getBlock(childNode), planTree, childNode, stack);
    stack.pop();
...
//      selectionNode.setChild(current);
      planTree.setChild(selectionNode, projectionNode);
      planTree.setChild(current, selectionNode);
    }
...
//      workingHavingNode.setChild(groupByNode);
      planTree.setChild(workingHavingNode, parentNode);
      planTree.setChild(groupByNode, workingHavingNode);
    }
  ...
  public LogicalNode visitGroupBy(FilterPushDownContext context, LogicalPlan plan,
                                  LogicalPlan.QueryBlock block, LogicalPlanTree planTree, GroupbyNode groupbyNode,
                                  Stack<LogicalNode> stack) throws PlanningException {
    LogicalNode parentNode = stack.peek();
    List<EvalNode> aggrEvals;
    if (parentNode.getType() == NodeType.HAVING) {
      aggrEvals = addHavingNode(context, plan, block, planTree, null, (HavingNode)parentNode, groupbyNode);
    } else {
      aggrEvals = addHavingNode(context, plan, block, planTree, parentNode, null, groupbyNode);
    }
...
//    findCanPushdownAndTransform(context, groupbyNode,groupbyNode.getChild(), notMatched, null, false, 0);
        findCanPushdownAndTransform(context, groupbyNode, planTree.getChild(groupbyNode), notMatched, null, false, 0);

    context.setFiltersTobePushed(tranformed.keySet());
    LogicalNode current = super.visitGroupBy(context, plan, block, planTree, groupbyNode, stack);
