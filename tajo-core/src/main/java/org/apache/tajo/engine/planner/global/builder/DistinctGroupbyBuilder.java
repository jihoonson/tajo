...
public class DistinctGroupbyBuilder {
  ...
  public ExecutionBlock buildPlan(GlobalPlanContext context,
    ...
    try {
      ...
      latestExecBlock.setPlan(plan.getLogicalNodeTree(), firstStageDistinctNode);
...
//      secondStageDistinctNode.setChild(scanNode);
      ...
      secondStageBlock.setPlan(plan.getLogicalNodeTree(), secondStageDistinctNode);

      context.getPlan().addConnect(channel);

      if (GlobalPlanner.hasUnionChild(context, firstStageDistinctNode)) {
...
//    baseDistinctNode.setChild(groupbyNode.getChild());
    plan.setChild(plan.getChild(groupbyNode), baseDistinctNode);
