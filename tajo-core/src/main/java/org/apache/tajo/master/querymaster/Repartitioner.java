...
public class Repartitioner {
  ...
  public static void scheduleFragmentsForJoinQuery(TaskSchedulerContext schedulerContext, SubQuery subQuery)
    ...
    LogicalPlanTree planTree = execBlock.getPlan();
    // If node is outer join and a preserved relation is empty, it should return zero rows.
    joinNode = PlannerUtil.findTopNode(planTree, execBlock.getRoot(), NodeType.JOIN);
    if (joinNode != null) {
      // find left top scan node
      ScanNode leftScanNode = PlannerUtil.findTopNode(planTree, planTree.getLeftChild(joinNode), NodeType.SCAN);
      ScanNode rightScanNode = PlannerUtil.findTopNode(planTree, planTree.getRightChild(joinNode), NodeType.SCAN);
