...
public class TestPlannerUtil {
  ...
  public final void testFindTopNode() throws CloneNotSupportedException, PlanningException {
    ...
    LogicalNode node = PlannerUtil.findTopNode(plan.getLogicalPlanTree(), root, NodeType.ROOT);
    assertEquals(NodeType.ROOT, node.getType());
    
    node = PlannerUtil.findTopNode(plan.getLogicalPlanTree(), root, NodeType.PROJECTION);
    assertEquals(NodeType.PROJECTION, node.getType());
    
    node = PlannerUtil.findTopNode(plan.getLogicalPlanTree(), root, NodeType.JOIN);
    assertEquals(NodeType.JOIN, node.getType());
    
    node = PlannerUtil.findTopNode(plan.getLogicalPlanTree(), root, NodeType.SCAN);
